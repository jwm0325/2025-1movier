...
# Todo1 지정된 장르를 포한한 영화중 평점이 높은 5개의 영화를 추천함, 단 추천인수가 5명이상
def ramdom_genres_items_best(genre):
    movies_df = pd.read_csv('data/movies_final.csv')
    genre_df = movies_df[movies_df['genres'].apply(lambda x : genre in x.lower())]
    genre_df = genre_df.fillna('')
    result_items = genre_df.query('rcount >= 5').sort_values(by='rmean',ascending=False).head(5).to_dict('records')
    return  result_items